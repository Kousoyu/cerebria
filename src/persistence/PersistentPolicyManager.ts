...
class PersistentPolicyManager {
  ...
  async createPolicy(policy: any) {
    ...
    if (this.db && this.usePersistence) {
      try {
        this.db.run('INSERT INTO policies (id, name, version, content, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)', [p.id, p.name, p.version, p.content, 1, p.createdAt, p.updatedAt]); 
      } catch (_) {} 
    }
    return p;
  }
  ...
  async updatePolicy(id: string, updates: any) {
    ...
    if (updates && updates.content) {
      p.content = JSON.stringify(updates.content);
    }
    p.version++;
    p.updatedAt = new Date().toISOString();
    if (this.db) {
      try {
        this.db.run('UPDATE policies SET name=?, content=?, version=?, updated_at=? WHERE id=?', [p.name, p.content, p.version, p.updatedAt, id]); 
      } catch (_) {} 
    }
    return p;
  }
  ...
  async deletePolicy(id: string) {
    ...
    if (this.db) {
      try {
        this.db.run('UPDATE policies SET is_active=0 WHERE id=?', [id]); 
      } catch (_) {} 
    }
    return true;
  }
